...
export class Chat3UserBotClient {
  ...
  async getDialogMembers(dialogId) {
    try {
      ...
      // Используем метод getDialogMembers из chat3-client
      // GET /api/dialogs/:dialogId/members
      const response = await this.client.getDialogMembers(dialogId);

      const members = response?.data || response || [];
      return { success: true, data: Array.isArray(members) ? members : [] };
    } catch (error) {
      console.error(`Ошибка при получении участников диалога ${dialogId}:`, error.message || error);
      return { success: false, error: error.message || String(error) };
    }
  }
  ...
  async getDialog(dialogId) {
    try {
      ...
      // Используем метод getDialog из chat3-client
      // GET /api/dialogs/:dialogId
      const response = await this.client.getDialog(dialogId);
      const dialog = response?.data || response;
      return dialog;
    } catch (error) {
      console.error(`Ошибка при получении диалога ${dialogId}:`, error.message || error);
      throw error;
    }
  }
   ...
   * @param {string} userId - ID пользователя
   * @param {Object} options - Опции (filter, limit, page, includeLastMessage)
   * @returns {Promise<Object>}
   */
  async getUserDialogs(userId, options = {}) {
    try {
      if (!this.client) {
        throw new Error('Chat3 API клиент не инициализирован');
      }

      // Используем метод getUserDialogs из chat3-client
      // GET /api/users/:userId/dialogs
      const params = {};
      if (options.filter) params.filter = options.filter;
      if (options.limit) params.limit = options.limit;
      if (options.page) params.page = options.page;
      if (options.includeLastMessage !== undefined) {
        params.includeLastMessage = options.includeLastMessage;
      }

      const response = await this.client.getUserDialogs(userId, params);
      return { success: true, data: response?.data || response || [] };
    } catch (error) {
      console.error(`Ошибка при получении диалогов пользователя ${userId}:`, error.message || error);
      return { success: false, error: error.message || String(error) };
    }
  }
}
