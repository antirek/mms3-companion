...
export class CompanionHandler {
  ...
  async generateSuggestion(clientMessage, contextMessages, fileIds, clientName) {
    try {
      ...
      // Формируем промпт для генерации подсказки
      const systemPrompt = `Ты - помощник менеджера по работе с клиентами. Твоя задача - анализировать запросы клиентов и предлагать профессиональные, вежливые и точные ответы.

Правила:
1. Ответ должен быть кратким, но информативным
...
5. Если есть прикрепленные файлы с данными, используй их для формирования ответа`;
      ...
      const userPrompt = `Клиент ${clientName} написал: "${clientMessage}"

Контекст диалога:
${contextText}

Сформируй подсказку для менеджера - профессиональный ответ на запрос клиента.`;
